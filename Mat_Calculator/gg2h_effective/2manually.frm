...
Local   A = ch*db(cix1,p1)*db(cix3,p2)*(d_(lix1,lix3)*p1.p2 - p1(lix3)*p2(lix1))*epol(lix1,p1)*epol(lix3,p2); 
Local  Ac = ch*db(cix101,p1)*db(cix103,p2)*(d_(lix101,lix103)*p1.p2 - p1(lix103)*p2(lix101))*epol(lix101,p1)*epol(lix103,p2); 
...
id epol(lix1?,p1?)*epol(lix101?,p1?) = - d_(lix1,lix101); 
*+(p1(lix1)*nv(lix101)+nv(lix101)*p1(lix1))/p1.nv;
*id epol(mu?,p?)*epol(b?,p?) = - d_(mu,b)+(p(mu)*nv(b)+nv(b)*p(mu))/p.nv;
.sort
print,A2;
.sort
