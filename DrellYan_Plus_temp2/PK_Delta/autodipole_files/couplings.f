...
c
c      open(unit=lun,file="Source/makefile",status='old',err=20)
c      dirup=0
c      goto 100
c 20   close(lun)

c      open(unit=lun,file="../Source/makefile",status='old',err=30)
c      dirup=1
c      goto 100
c 30   close(lun)

c      open(unit=lun,file="../../Source/makefile",status='old',err=40)
c      dirup=2
c      goto 100
c 40   close(lun)

c      open(unit=lun,file="../../../Source/makefile",status='old',err=50)
c      dirup=3
c      goto 100
c 50   close(lun)
...
c
c          if(index(filename,"_card").gt.0) then
c             tempname='/Cards/'//tempname(1:fine)
c             fine=fine+7
c      endif

      open(unit=lun,file=tempname(1:fine),status='old',err=110)
c      if(dirup.eq.0) open(unit=lun,file=tempname(1:fine),status='old',err=110)
c      if(dirup.eq.1) open(unit=lun,file='../'//tempname(1:fine),status='old',err=110)
c      if(dirup.eq.2) open(unit=lun,file='../../'//tempname(1:fine),status='old',err=110)
c      if(dirup.eq.3) open(unit=lun,file='../../../'//tempname(1:fine),status='old',err=110)
      return

 110  fopened=.false.
      close(lun)
      write (*,*) 'Warning: file ',tempname(1:fine)
c     & ,'is not in the  main directory'
