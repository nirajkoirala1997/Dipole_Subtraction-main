...
c       call Event(X1,X2,X4,flo2,virt,virtN)          !! Numerical BORN in EXACT
c       write(*,*)'test =',x2, tauh/x1
       BORN = flo2                                !! Numerical BORN in EXACT
       BORN = BORN*2d0*Q/RS/RS        !! why Q?`

        If(ORD.ge.1) Then
        virt1 = virt             !! virtual by born
        virt2 = virtN
        virt2 = virt2 - 1        !!-1 to prevent double counting of born
        endif
...
c        FLUX = (  (FU1*FU2B+FU1B*FU2+FCH1*FCH2B+FCH1B*FCH2)*
c     &          (gau**4+6.0d0*gau**2*gvu**2+gvu**4)
c     &    + (FD1*FD2B+FD1B*FD2+FST1*FST2B+FST1B*FST2+FBOT1*FBOT2B
c     &    + FBOT1B*FBOT2)*
c     &     (gad**4+6.0d0*gad**2*gvd**2+gvd**4)   )
     ...
     &    )
        ...
        ! FLUX FORM NF diagram PART IN 2 LOOP
        If(ORD.eq.2) THEN
        FLUX = (  (FU1*FU2B+FU1B*FU2+FCH1*FCH2B+FCH1B*FCH2)*
     &          (gau**2+gvu**2)
     &    + (FD1*FD2B+FD1B*FD2+FST1*FST2B+FST1B*FST2+FBOT1*FBOT2B
     ...
     &     (gad**2+gvd**2)   )
        ...
        REG = BORN*FLUX*xINT2*XJAC/2D0/XEPS/X1/X2 + REG
        ENDIF
       RETURN
