...
c      alphsmz = alphasPDF(XMZ)
      WRITE(*,*)"Alphas(mZ) =",alphsmz
c      nkind = 2    !! what is this ?
c      AS = alphas(nkind,ord,xmz,prefix,alphsmz,xmur)/4.0d0/PI
      AS = alphasPDF(XMUR)/4.0d0/PI
...
!!!!---------------- writing into files (same structure) ----------------------!!!!
      ...
      DO IPDF = IPDFI, IPDFF   !! PDF Variation
       ...
       open(unit=115,file='kfactor.dat',status='unknown')
...
c        WRITE(*,*)'***********(2,2)**********'
      ...
      sk1 = SIGPP1(IPDF,IQ,ISCALE)/SIGPP0(IPDF,IQ,ISCALE) 
      sk2 = SIGPP2(IPDF,IQ,ISCALE)/SIGPP0(IPDF,IQ,ISCALE) 
      sk3 = SIGPP3(IPDF,IQ,ISCALE)/SIGPP0(IPDF,IQ,ISCALE) 
     ...
     &                          error3(IPDF,IQ,ISCALE) 
      ...
      write(115,101)Q,XMUR,XMUF,sk1,sk2,sk3
 101  format(6(f10.4,2x))
