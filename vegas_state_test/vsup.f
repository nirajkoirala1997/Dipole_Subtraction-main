      subroutine vegas_recover(IT,TI,TSI,AVGI,SD,CHI2A)
      ...
      itmx=its
      plot=.false.
      if (nprn .eq. 1 ) open(unit=ilabel,file=fname8,status='unknown')
...
C     
         ...
         IF(NPRN.NE.0) WRITE(ilabel,200) NDIM,CALLS,IT,ITMX,MDS,ND
...
C         
        if (ireset .eq. 1 ) call vegas_recover(IT,TI,TSI,AVGI,SD,CHI2A)
         IT=IT+1
         TI =0D0
         TSI=0D0
         DO J=1,NDIM
...
C     
         IF(NPRN.NE.0) THEN
            ...
            CALL BRM48O(IJKLUT, NTOTUT, NTOT2T)
            OPEN(UNIT=10, FILE='state.dat', STATUS='REPLACE')
            WRITE(10, *) IJKLUT, NTOTUT, NTOT2T
            CLOSE(10)


	WRITE(ilabel,201) IT,TI,TSI,AVGI,SD,CHI2A
        FLUSH(ilabel)
         ...
         IF(NPRN.LT.0) THEN
            ...
            WRITE(ilabel,202) J,(XI(I,J),DI(I,J),D(I,J),I=1,ND)
            ENDDO
         ENDIF
      ENDIF
C     
