...
! Kb = Kbar      
c----------------------------------------------------------- 
      subroutine getPK(iplus,x,xmuf,p,xp1,xp2,SumP)
...
c        Aplus= Alp*(Pqqp(x)+AKbarP_qq(x)+AKtilP_qq(x))*coef
        Aplus= Alp*PqqP(x)*coef
        ALLP(k) = Aplus
c        write(*,*)'Aplus =', Aplus

c       elseif (iplus .eq. 1) then
c       Areg = (AKbarreg_qq(x)+AKtilreg_qq(x))*coef
c       AllP(k)= Alp*Areg
c       elseif (iplus .eq. 0) then
c       ADel = (AKbarD_qq(x)+AKtilD_qq(x))*coef
c       AllP(k)= Alp*ADel
       endif
